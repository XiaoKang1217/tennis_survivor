...
function playerDisplayName(entry, fallback = '球员姓名待更新') {
...
function h2hSearchOptions(value) {
  ...
  return entries.map(entry => {
    ...
    const name = playerDisplayName(entry, fact(entry.displayNameOriginal) || '球员姓名待更新');
...
function leaderboardEntries(value) {
  ...
  return entries.map(entry => {
    ...
    return Object.freeze({
      ...
      countryCode,
      countryMark: flag(countryCode),
      ageLabel: Number.isFinite(Number(age)) ? `${Number(age)}岁` : '年龄待更新',
      position,
  ...
  const history = Array.isArray(payload.history) ? payload.history
    .map(match => {
      ...
      return {
        ...
        occurredOn: dateText,
        dateText,
        tournamentName: match.tournamentNameZh || match.tournamentName || '赛事待确认',
        levelLabel,
        surfaceLabel,
        roundLabel,
        ...
        winnerName,
        winnerLabel: winnerName ? `赢家 ${winnerName}` : ''
      };
    })
...
Page({
    ...
    try {
      ...
      if (cached?.payload) {
        this.setData({
          ...
          failed: false,
          deliveryState: 'stale',
          deliveryMessage: '刷新失败，继续显示本地可信球员列表'
        });
        return;
      }
      this.setData({
    ...
    try {
      ...
      if (cached?.payload) {
        ...
        this.setData({
          ...
          h2hFailed: false,
          h2hMessage: cachedResult ? '显示本地可信 H2H，正在后台更新' : '暂未找到这组 H2H',
      ...
      this.setData({
        ...
        h2hEmpty: notFound,
        h2hFailed: !notFound,
        h2hMessage: notFound ? '暂未找到这组 H2H' : 'H2H 暂不可用',
        h2hResult: null
      });
    }
  },
  ...
  openPlayer(event) {
    ...
    wx.navigateTo({
      url: `/pages/player-detail/index?playerId=${encodeURIComponent(player.id)}`
