...
test('following player badge visibly hydrates four-level next appearance copy in one batch', () => {
  ...
  assert.doesNotMatch(script, /8 \* 60 \* 60 \* 1000/u);
  assert.match(script, /roundLabelZh/u);
  assert.match(script, /roundLabel\(appearance\.roundCode/u);
  assert.match(markup, /catchtap="openNextPlan"/u);
  assert.match(script, /下一站 ·/u);
  assert.match(script, /近期暂无明确参赛安排/u);
