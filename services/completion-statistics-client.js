...
class CompletionStatisticsClient {
  ...
  setTransportState(value) {
    if (!TRANSPORT_STATES.includes(value)) {
      throw new Error('completion statistics transport state invalid');
    }
  ...
  async fetchSnapshot(options = {}) {
    ...
    const request = this.http.request(
      `/api/v1/bff/matches/${encodeURIComponent(this.matchId)}/completion${refreshQuery}`,
      { noCache: force }
    ).then(value => {
      const projection = contracts.completionProjection(value);
      const result = this.store.snapshot(projection);
      if (result.action === 'resync_required') {
        throw new Error('completion_statistics_conflict');
      }
      safeEvents.emit('completion_statistics_snapshot_received');
      return projection;
    }).catch(error => {
      if (error.statusCode === 404 && this.store.projection === null) return null;
      throw error;
  ...
  async openRealtime() {
    ...
    const parser = new SseParser(event => {
      ...
      try {
        const frame = contracts.completionRealtimeFrame(JSON.parse(event.data));
        const result = this.store.frame(frame);
        if (result.action === 'resync_required') void this.resync();
        else if (result.action === 'unavailable') this.setTransportState('connected');
