-- Seoul 250 + Singapore 500: event-based dual Combo, daily delta settlement.
-- Requires the existing US Open welfare migration. Does not settle or credit wallets on install.
begin;

create or replace function public.tour_manager_apply_seoul_singapore_combo_v1(
  p_station_key text,
  p_season int
)
returns int
...
begin
  ...
  select
    ...
    coalesce((metadata #>> '{combo,steady,cap}')::int, 150),
    ...
    coalesce((metadata #>> '{combo,value_pick,QF}')::int, 60),
    coalesce((metadata #>> '{combo,value_pick,SF}')::int, 120),
    coalesce((metadata #>> '{combo,value_pick,F}')::int, 200),
    coalesce((metadata #>> '{combo,value_pick,W}')::int, 300),
  ...
  if v_combo_version is distinct from 'seoul_singapore_2026_v1' then
    raise exception 'seoul_singapore_combo_required:%', coalesce(v_combo_version, 'missing');
  end if;

  select metadata #>> '{combo,dual_tour,event_keys,0}', metadata #>> '{combo,dual_tour,event_keys,1}'
  into v_left_event, v_right_event
  from public.tour_manager_station_configs
  where station_key = p_station_key and season = p_season;
  if v_left_event is null or v_right_event is null or v_left_event = v_right_event then
    raise exception 'two_distinct_combo_events_required';
  end if;
  ...
  loop
    ...
    select
      ...
      count(*) filter (where is_active and event_key = v_left_event and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('QF')),
      count(*) filter (where is_active and event_key = v_right_event and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('QF')),
      count(*) filter (where is_active and event_key = v_left_event and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('SF')),
      count(*) filter (where is_active and event_key = v_right_event and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('SF')),
      count(*) filter (where is_active and event_key = v_left_event and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('F')),
      count(*) filter (where is_active and event_key = v_right_event and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('F')),
      count(*) filter (where is_active and event_key = v_left_event and reached_round = 'W'),
      count(*) filter (where is_active and event_key = v_right_event and reached_round = 'W'),
      coalesce(max(case
    ...
    if v_dual_round is not null then
      ...
      from (
        ...
        where lineup_id = v_lineup.id
          and is_active
          and event_key in (v_left_event, v_right_event)
          and case
    ...
    v_original_cost := coalesce(v_lineup.original_lineup_cost, v_lineup.lineup_cost);
    if v_original_cost > 0 and v_original_cost <= v_small_max_cost then
      v_small_multiple := v_gross::numeric / v_original_cost::numeric;
      if v_gross * 4 >= v_original_cost * 6 then
        v_small_bonus := 200;
      elsif v_gross * 4 >= v_original_cost * 5 then
        v_small_bonus := 150;
      elsif v_gross >= v_original_cost then
        v_small_bonus := 100;
      elsif v_gross * 4 >= v_original_cost * 3 then
        v_small_bonus := 50;
      end if;
    end if;
...
revoke all on function public.tour_manager_apply_seoul_singapore_combo_v1(text, int)
  from public, anon, authenticated;
grant execute on function public.tour_manager_apply_seoul_singapore_combo_v1(text, int)
  to service_role;
...
begin
  ...
  if v_combo_version = 'seoul_singapore_2026_v1' then
    return public.tour_manager_apply_seoul_singapore_combo_v1(p_station_key, p_season);
  end if;
...
begin
  ...
  return jsonb_build_object(
    'welfare_uses_this_season', (
      select count(*)::int from public.tour_manager_lineups
      where user_id = v_user and season = p_season and welfare_discount > 0
