-- 巡回赛经纪人：换入球员从换入基准轮次开始展示和后续结算。

update public.tour_manager_lineup_players
set reached_round = metadata->>'transfer_baseline_round'
where is_transfer = true
  and is_active = true
  and coalesce(reached_round, 'OUT') = 'OUT'
  and coalesce(metadata->>'transfer_baseline_round', 'OUT') <> 'OUT'
  and public.tour_manager_round_order(metadata->>'transfer_baseline_round') > public.tour_manager_round_order('OUT');
