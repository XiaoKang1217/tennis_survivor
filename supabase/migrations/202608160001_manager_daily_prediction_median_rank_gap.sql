-- Select the upper-median ranking-gap match from 2026-08-17 onward.
-- Existing station/date/tour questions remain immutable.
...
begin
  ...
  foreach v_tour in array array['ATP','WTA'] loop
    ...
    where ranking_gap_position = case
      when p_contest_date >= date '2026-08-17'
        then floor(candidate_count::numeric / 2)::int + 1
      else 1
    end
    limit 1;
    ...
    insert into public.tour_manager_daily_prediction_games (
      ...
      v_match.ranking_gap, 10,
      case
        when p_contest_date >= date '2026-08-17' then 'median_world_rank_gap_official_event_day'
        else 'closest_world_rank_official_event_day'
      end
    )
