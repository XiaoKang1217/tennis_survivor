-- Cincinnati 2026: publish the shared ATP/WTA transfer window and preserve
-- the frozen Cincinnati village-hope contract through transfers.

update public.tour_manager_events
set
  transfer_window_opens_at = '2026-08-15T10:00:00+08:00'::timestamptz,
  transfer_window_closes_at = '2026-08-15T22:45:00+08:00'::timestamptz,
  transfer_window_note = '本站换人窗口为 08/15 10:00 - 08/15 22:45；ATP/WTA 同一窗口开放，男女可互换。换人不支持自定义全村的希望：若换下提交时冻结的全村希望，换入球员自动继承全村希望；否则全村希望保持不变。',
  transfer_fee_rate = 0.15,
  metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('cross_tour_transfer', true),
  updated_at = now()
where station_key = '2026-w33-cincinnati'
  and season = 2026
  and tour in ('ATP', 'WTA')
  and event_key in (
    'atp-2026-w33-cincinnati-cincinnati-open',
    'wta-2026-w33-cincinnati-cincinnati-open'
  );
...
create or replace function public.tour_manager_apply_cincinnati_transfer_village_hope()
returns trigger
language plpgsql
security definer
...
declare
  ...
  v_in_name text;
begin
  if new.station_key <> '2026-w33-cincinnati' then
    return new;
  end if;
  ...
  if coalesce(v_lineup_village_key, '') <> '' and v_lineup_village_key = v_out_key then
    ...
    set village_hope_player_key = v_in_key,
        ...
        updated_at = now()
    where id = new.lineup_id
      and station_key = '2026-w33-cincinnati';
...
drop trigger if exists tour_manager_cincinnati_transfer_village_hope_guard on public.tour_manager_transfers;
create trigger tour_manager_cincinnati_transfer_village_hope_guard
after insert on public.tour_manager_transfers
for each row execute function public.tour_manager_apply_cincinnati_transfer_village_hope();
...
begin
  ...
  where station_key = '2026-w33-cincinnati'
    ...
    and transfer_window_opens_at = '2026-08-15T10:00:00+08:00'::timestamptz
    and transfer_window_closes_at = '2026-08-15T22:45:00+08:00'::timestamptz
    and transfer_fee_rate = 0.15
    and lower(coalesce(metadata->>'cross_tour_transfer', 'false')) in ('true', '1', 'yes');

  if v_count <> 2 then
    raise exception 'cincinnati_transfer_window_update_incomplete: expected 2 events, got %', v_count;
  end if;
end
$$;
