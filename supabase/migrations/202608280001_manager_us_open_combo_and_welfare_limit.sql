-- US Open 2026: config-driven welfare limit and result Combo.
--
-- Welfare remains an instant signing discount, independent from result Combo.
-- The season allowance is counted from discounted lineups submitted within
-- the configured season year, including cancelled lineups.
...
begin
  ...
  if jsonb_typeof(v_combo->'welfare') = 'object' then
    ...
    where user_id = v_user
      and season = p_season
      and submitted_at >= make_timestamptz(p_season, 1, 1, 0, 0, 0, 'UTC')
      and submitted_at < make_timestamptz(p_season + 1, 1, 1, 0, 0, 0, 'UTC')
  ...
  v_village_selection := coalesce(v_combo #>> '{village_hope,selection}', '');
  if v_village_selection = 'highest_original_price_at_submission' then
    with req as (
      select distinct x->>'event_key' as event_key, x->>'player_key' as player_key
      from jsonb_array_elements(p_contracts) x
    )
...
create or replace function public.tour_manager_apply_us_open_combo_v1(
  p_station_key text,
  p_season int
)
returns int
...
declare
  ...
  v_steady_context jsonb;
  v_multi_round text;
  v_dual_round text;
  v_jewel_round text;
  v_village_round text;
  v_jewel_player_name text;
  v_village_player_name text;
begin
  ...
  if v_combo_version is distinct from 'us_open_2026_v1' then
    raise exception 'us_open_combo_v1_required:%', coalesce(v_combo_version, 'missing');
  end if;

  v_combo_cap := coalesce((v_combo->>'total_cap')::int, 1200);
  v_steady_min_players := coalesce((v_combo #>> '{steady,min_players}')::int, 3);
  v_value_max_price := coalesce((v_combo #>> '{value_pick,max_price}')::int, 300);
  ...
  loop
    ...
    v_jewel_player_name := null;
    v_village_player_name := null;
    ...
    select
      ...
      count(*) filter (where is_active and tour = 'ATP' and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('R32')),
      count(*) filter (where is_active and tour = 'WTA' and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('R32')),
    ...
    into
      ...
      v_atp_r32, v_wta_r32, v_atp_r16, v_wta_r16, v_atp_qf, v_wta_qf, v_atp_sf, v_wta_sf, v_atp_f, v_wta_f, v_atp_w, v_wta_w,
      v_gross
    from public.tour_manager_lineup_players
    where lineup_id = v_lineup.id;

    if v_contract_count >= v_steady_min_players then
      ...
      if v_multi_round is not null then
        v_multi_bonus := coalesce((v_combo #>> array['steady', 'multi', v_multi_round])::int, 0);
        v_steady_context := v_steady_context || jsonb_build_array(v_multi_round);
        ...
        from (
          ...
          where lineup_id = v_lineup.id
            and is_active
            and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order(v_multi_round)
        ) qualified;
      end if;

      if v_contract_count > 0
        and v_r16_count = v_contract_count
        and coalesce((v_combo #>> '{steady,all_r16}')::int, 0) > 0
      then
        v_all_r16_bonus := coalesce((v_combo #>> '{steady,all_r16}')::int, 0);
        v_steady_context := v_steady_context || jsonb_build_array('全员R16');
        if v_multi_round is null then
          select coalesce(jsonb_agg(player_name order by created_at), '[]'::jsonb)
          into v_steady_players
          from (
            select coalesce(nullif(name_zh, ''), nullif(name_en, ''), player_key) player_name, created_at
    ...
    v_stable_bonus := v_multi_bonus + v_all_r16_bonus;
    ...
    if v_atp_w > 0 and v_wta_w > 0 then
      ...
      v_dual_round := 'R16';
    elsif v_atp_r32 > 0 and v_wta_r32 > 0 then
      v_dual_round := 'R32';
    end if;
    ...
    if v_dual_round is not null then
      ...
      from (
        ...
        where lineup_id = v_lineup.id
          and is_active
          and tour in ('ATP', 'WTA')
          and public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order(v_dual_round)
      ) qualified;
    end if;

    select player_name, reached_round, bonus
    into v_jewel_player_name, v_jewel_round, v_jewel_bonus
    from (
      select
        coalesce(nullif(name_zh, ''), nullif(name_en, ''), player_key) player_name,
        reached_round,
        case
          when reached_round = 'W' then coalesce((v_combo #>> '{value_pick,W}')::int, 1200)
          when public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('F') then coalesce((v_combo #>> '{value_pick,F}')::int, 1000)
          when public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('SF') then coalesce((v_combo #>> '{value_pick,SF}')::int, 800)
          when public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('QF') then coalesce((v_combo #>> '{value_pick,QF}')::int, 600)
          when public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('R16') then coalesce((v_combo #>> '{value_pick,R16}')::int, 400)
          when public.tour_manager_round_order(coalesce(reached_round, 'OUT')) >= public.tour_manager_round_order('R32') then coalesce((v_combo #>> '{value_pick,R32}')::int, 200)
          else 0
        end bonus,
        public.tour_manager_round_order(coalesce(reached_round, 'OUT')) round_order,
        created_at
      from public.tour_manager_lineup_players
      where lineup_id = v_lineup.id
        and is_active
    ...
    where bonus > 0
    order by bonus desc, round_order desc, created_at
    limit 1;
    v_jewel_bonus := coalesce(v_jewel_bonus, 0);
    if v_jewel_bonus > 0 then
      v_jewel_players := jsonb_build_array(v_jewel_player_name);
    end if;

    select
      coalesce(nullif(name_zh, ''), nullif(name_en, ''), player_key),
      reached_round
    into v_village_player_name, v_village_round
    from public.tour_manager_lineup_players
    where lineup_id = v_lineup.id
    ...
    if v_village_round = 'W' then
      v_village_bonus := coalesce((v_combo #>> '{village_hope,W}')::int, 1200);
    elsif public.tour_manager_round_order(coalesce(v_village_round, 'OUT')) >= public.tour_manager_round_order('F') then
      v_village_bonus := coalesce((v_combo #>> '{village_hope,F}')::int, 1000);
    elsif public.tour_manager_round_order(coalesce(v_village_round, 'OUT')) >= public.tour_manager_round_order('SF') then
      v_village_bonus := coalesce((v_combo #>> '{village_hope,SF}')::int, 800);
    elsif public.tour_manager_round_order(coalesce(v_village_round, 'OUT')) >= public.tour_manager_round_order('QF') then
      v_village_bonus := coalesce((v_combo #>> '{village_hope,QF}')::int, 600);
    elsif public.tour_manager_round_order(coalesce(v_village_round, 'OUT')) >= public.tour_manager_round_order('R16') then
      v_village_bonus := coalesce((v_combo #>> '{village_hope,R16}')::int, 400);
    elsif public.tour_manager_round_order(coalesce(v_village_round, 'OUT')) >= public.tour_manager_round_order('R32') then
      v_village_bonus := coalesce((v_combo #>> '{village_hope,R32}')::int, 200);
    end if;
    if v_village_bonus > 0 then
      v_village_players := jsonb_build_array(v_village_player_name);
    end if;
    ...
    if v_stable_bonus > 0 then
      v_combo_details := v_combo_details || jsonb_build_array(jsonb_build_object(
        ...
        'players', v_steady_players, 'context', v_steady_context
      ));
    end if;
...
revoke all on function public.tour_manager_apply_us_open_combo_v1(text, int)
  from public, anon, authenticated;
grant execute on function public.tour_manager_apply_us_open_combo_v1(text, int)
  to service_role;
...
begin
  ...
  if v_combo_version = 'us_open_2026_v1' then
    return public.tour_manager_apply_us_open_combo_v1(p_station_key, p_season);
  end if;
