-- 巡回赛经纪人：发布巴斯塔德 + 雅典换人窗口，并按赛事 metadata 放行同站 ATP/WTA 互换。
-- 保留换人净返还优先恢复本站签约金额度、轮次基准和钱包流水规则。

update public.tour_manager_events
set transfer_window_opens_at = '2026-07-15T06:00:00+08:00'::timestamptz,
    transfer_window_closes_at = '2026-07-15T17:00:00+08:00'::timestamptz,
    transfer_window_note = '本站换人窗口为 07/15 06:00 - 07/15 17:00；ATP/WTA 同一窗口开放，ATP/WTA 可以互换。',
    metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object('cross_tour_transfer', true),
    updated_at = now()
where station_key = '2026-w29-bastad-athens'
  and event_key in (
    'atp-2026-w29-bastad-nordea-open',
...
begin
  ...
  if v_in.event_key <> v_out.event_key and not (
    lower(coalesce(v_out_event.metadata->>'cross_tour_transfer', 'false')) in ('true','1','yes')
    and lower(coalesce(v_in_event.metadata->>'cross_tour_transfer', 'false')) in ('true','1','yes')
  ) then
