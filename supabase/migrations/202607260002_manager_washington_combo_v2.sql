-- Washington Combo v2 changes only the four value-pick awards. Keep the
-- frozen v1 calculator, the pre-Washington legacy calculator, the serialized
-- daily entitlement-minus-paid delta, and the ledger-detail schema intact.

create or replace function public.tour_manager_apply_washington_combo_v2(
  p_station_key text,
  p_season int
)
returns int
...
begin
  ...
  select
    ...
    coalesce((metadata #>> '{combo,value_pick,QF}')::int, 50),
    coalesce((metadata #>> '{combo,value_pick,SF}')::int, 100),
    coalesce((metadata #>> '{combo,value_pick,F}')::int, 150),
    coalesce((metadata #>> '{combo,value_pick,W}')::int, 200),
  ...
  if v_combo_version is distinct from 'washington_2026_v2' then
    raise exception 'washington_combo_v2_required:%', coalesce(v_combo_version, 'missing');
  end if;
...
revoke all on function public.tour_manager_apply_washington_combo_v2(text, int)
  from public, anon, authenticated;
grant execute on function public.tour_manager_apply_washington_combo_v2(text, int)
  to service_role;
