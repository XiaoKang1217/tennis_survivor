...
const TARGET_DATE = '2026-08-16';
const STATION_KEY = '2026-w33-cincinnati';
const EVENT_KEY = 'atp-2026-w33-cincinnati-cincinnati-open';
const EXPIRED_MATCH_KEY = `${EVENT_KEY}:MS032`;
const REPLACEMENT_MATCH_KEY = `${EVENT_KEY}:MS049`;
...
if (today !== TARGET_DATE) {
  console.log(`Skipped Cincinnati ATP prediction replacement: target=${TARGET_DATE} today=${today}`);
  process.exit(0);
}
...
if (games.length !== 1) {
  throw new Error(`Expected one Cincinnati ATP prediction for ${TARGET_DATE}, got ${games.length}`);
}

const game = games[0];
if (game.match_key === REPLACEMENT_MATCH_KEY) {
  console.log(`Cincinnati ATP prediction already uses ${REPLACEMENT_MATCH_KEY}`);
  process.exit(0);
}
if (game.match_key !== EXPIRED_MATCH_KEY) {
  throw new Error(`Refusing to replace unexpected Cincinnati ATP prediction ${game.match_key}`);
}
if (game.status !== 'open') {
  throw new Error(`Refusing to replace Cincinnati ATP prediction with status ${game.status}`);
}
...
const sourceMatches = await client.select('tour_manager_matches', {
  event_key: `eq.${EVENT_KEY}`,
  match_key: `eq.${EXPIRED_MATCH_KEY}`,
  select: 'match_key,status,winner_key,scheduled_at'
});
if (sourceMatches.length !== 1 || sourceMatches[0].status !== 'completed' || !sourceMatches[0].winner_key) {
  throw new Error(`Expired ATP match is not confirmed complete: ${JSON.stringify(sourceMatches[0] || null)}`);
}
...
if (match.raw?.date !== TARGET_DATE) {
  throw new Error(`Replacement ATP match has unexpected official date ${match.raw?.date || ''}`);
}
...
// Choices from the completed match cannot be mapped fairly to a new matchup.
// The expired question cannot accept new submissions, so clearing before the
// guarded update is safe and makes every participant choose again.
if (picks.length) {
...
const replacement = {
  ...
  selection_method: 'manual_replacement_expired_question_20260816',
  status: 'open',
  winner_key: null,
  winner_name: null,
  settled_at: null,
...
if (updated.length !== 1 || updated[0].match_key !== REPLACEMENT_MATCH_KEY) {
  throw new Error('Cincinnati ATP prediction replacement did not update exactly one game');
}
