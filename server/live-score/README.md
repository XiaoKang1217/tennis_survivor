...
- API Tennis 赛程基线缓存 6 小时；WTA 官方赛程每 5 分钟复核一次，当前已核实的 ATP 官方日程作为完整白名单随代码发布。
...
- ATP 官方 Order of Play 用作完整白名单；已核实的 2026-07-22、2026-07-23 埃斯托利尔和基茨比厄尔日程逐场写入对阵、球场、顺序和官方赛程日。API Tennis 仅为这些白名单对阵提供 event ID、赔率和实时比分。
...
1. `get_fixtures` 创建候选赛程基线。
2. ATP/WTA 官方完整日程按“巡回赛＋官方赛程日＋完整双方球员＋单双打”逐场核对；官方清单决定赛程是否存在、赛事归属、球场、场地和顺序。
3. `get_livescore` 只能覆盖白名单比赛的状态、盘分、局内分、发球方和最后一分，不得新增比赛或修改日期、赛事、球场、场地和球员。
4. 中文本地化只替换球员显示名；赛事事实字段在进入本地化层之前已经冻结。
